...
export const CriarPredioSchema = z.object({
  nome: z.string().min(2, "O nome do prédio deve ter pelo menos 2 caracteres.").trim(),
});
...
export const CriarEspacoSchema = z.object({
  nome: z.string().min(2, "O nome do espaço deve ter pelo menos 2 caracteres.").trim(),
  capacidade: z.number().int().positive("A capacidade deve ser maior que zero."),
  

  tipo: z.nativeEnum(TipoEspaco, {
    required_error: "O tipo é obrigatório.",
    invalid_type_error: "O tipo deve ser obrigatoriamente 'sala' ou 'laboratorio'.",
  }),
